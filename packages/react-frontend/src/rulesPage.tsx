...
export default function RulesPage() {
  ...
  const [newRuleStatus, setNewRuleStatus] = useState<"PENDING" | "CONFIRMED" | "CANCELLED">("PENDING");
  ...
  if (loading) {
    return <div className="background-house min-h-screen flex items-center justify-center">Loading...</div>;
  }
      ...
      <div className="flex-1 flex flex-col items-center p-4">
        <div className="panel w-full max-w-2xl">
          ...
          <div className="space-y-4">
            {rules.length === 0 ? (
              <p className="text-center text-text/70 py-6">No rules yet — add one!</p>
            ) : (
              rules.map((rule) => (
                <div key={rule._id} className="bg-primary/40 rounded-lg p-4 shadow-sm relative">
                  <div className="flex justify-between items-start">
                    <p className="text-text font-secondary text-lg">{rule.description}</p>
                  ...
                  <p className="text-sm mt-2 text-text/80">Status: {rule.status}</p>
                </div>
              ))
            )}
          </div>
          ...
          <div className="mt-6 pt-4 border-t border-text/20">
            ...
            <form id="add-rule-form" onSubmit={handleAddRule} className="flex flex-col gap-3">
              <textarea
                value={newRuleDesc}
              ...
              <div className="flex items-center gap-2">
                <label htmlFor="status" className="text-text font-medium">Status:</label>
                <select
                  id="status"
                  value={newRuleStatus}
                  onChange={(e) =>
                    setNewRuleStatus(e.target.value as "PENDING" | "CONFIRMED" | "CANCELLED")
                  }
        ...
        {error && <p className="text-red-600 bg-red-100/50 px-4 py-2 rounded mt-6">{error}</p>}
      </div>
    </div>
  );
}