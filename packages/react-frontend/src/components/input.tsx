...
export type FieldsLayout<T extends { settings: Record<string, any> }> = Record<
	string,
	AttributeFieldProps<T>
>;
...
export function InputField<T extends { settings: Record<string, any> }>({
	fieldName,
	state,
}: {
	fieldName: AttributeFieldProps<T>;
							...
							return (
								<button
									...
									className={`w-14 h-7 flex items-center rounded-full transition-colors duration-300 ${
										state.draft.settings[f.field] ===
										f.onName
											? "bg-primary justify-end" // light mode (right)
											: "bg-primary justify-start" // dark mode (left)
									}`}
								>
